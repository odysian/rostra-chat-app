...
- **Neon URL split handling:** Keep `settings.DATABASE_URL` in libpq form for sync consumers like Alembic/psycopg2. The async runtime path derives a separate `postgresql+asyncpg` config, strips libpq-only query params such as `sslmode` and `channel_binding`, and passes TLS through asyncpg's `ssl` connect arg instead.
